...
/// Try to determine if `c` is ni the Mandlebrot set, using at most `limit`
...
/// the character given by the `sperator` argument, and <left> and <right> are both
...
fn main() {
    ...
    if args.len() != 5 {
        ...
        "Usage: mangelbrot FILE PIXELS UPPERLEFT LOWERRIGHT").unwrap();
        writeln!(std::io::stderr(), 
